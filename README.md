...
- 颜色变化按 `occurred_at` 在批次内形成带 `seq` 的时间链；批量补录（含倒序录入）后整体重排，链尾是唯一当前色。
- 颜色变化的“变化前颜色”默认沿用上一条链结果色，显式录入的人工证据不被重排覆盖。
- 误录颜色采用软删除：行保留、链序压实、前后衔接和当前色在同一事务修复；有附件时需先删照片。
- 批次归档后颜色记录只读，禁止任何改色操作与证据上传。
- 颜色变化附件区分变化前（BEFORE）与变化后（AFTER）证据。
- 计划用料不扣库存，实际消耗才扣库存。
