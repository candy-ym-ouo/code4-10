...
- 消耗、损耗、撤销和反向库存流水。
- 颜色变化时间线和批次当前颜色。
- 变化前/变化后照片证据、颜色变化批量补录（倒序录入仍确定唯一当前色）。
- 误录颜色记录软作废，时间链与证据不被物理删除破坏。
...
验收脚本会真实创建来源、材料、批次、项目、需求、消耗和颜色变化，验证余额从 1000g 降到 500g、撤销后恢复 1000g、最新流水为 `REVERSAL`；颜色部分会验证倒序批量补录后当前色唯一、作废误录后当前色回退且时间链保留。
...
- 颜色时间线以批次内 `(occurred_at, seq)` 排序：单条与批量补录后均重排 seq，同一时间戳也有确定先后，当前色唯一。
- 批量补录接受任意顺序（含完全倒序），服务端归并重排时间链并自动链接前后色。
- 误录颜色记录只能作废（软删除），行、照片证据与审计全部保留；作废后按时间链重算当前色。
- 颜色证据必须标记为变化前（BEFORE）或变化后（AFTER）。
- 已归档批次的颜色时间线为只读，禁止新增、补录、改色、作废或上传证据。
- 计划用料不扣库存，实际消耗才扣库存。
