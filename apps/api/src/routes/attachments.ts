...
export async function attachmentRoutes(app: FastifyInstance): Promise<void> {
  app.post("/attachments", async (request, reply) => {
    ...
    if (fields.phase) {
      if (!attachmentPhases.includes(fields.phase as AttachmentPhase)) {
        throw new AppError(422, "INVALID_PHASE", "证据阶段只能是 BEFORE 或 AFTER");
      }
      phase = fields.phase as AttachmentPhase;
    }
    if (ownerType === "COLOR_CHANGE" && !phase) {
      throw new AppError(422, "PHASE_REQUIRED", "颜色变化证据必须标记为变化前或变化后");
    }
      ...
      const created = await withTransaction(async (client) => {
        ...
        if (ownerType === "COLOR_CHANGE") {
          const locked = await client.query(
            `SELECT b.status, c.voided_at FROM color_changes c
               JOIN batches b ON b.id = c.batch_id
              WHERE c.id = $1 FOR UPDATE OF c`,
            [fields.ownerId]
          );
          if (!locked.rowCount) throw new AppError(422, "INVALID_OWNER", "颜色变化记录不存在");
          if (locked.rows[0]?.status === "ARCHIVED") throw new AppError(409, "BATCH_ARCHIVED", "批次已归档，证据为只读");
          if (locked.rows[0]?.voided_at) throw new AppError(409, "COLOR_CHANGE_VOIDED", "已作废记录不能再上传证据");
        }
  ...
  app.delete<{ Params: { id: string } }>("/attachments/:id", async (request, reply) => {
    ...
    try {
      await withTransaction(async (client) => {
        if (attachment.owner_type === "COLOR_CHANGE") {
          const locked = await client.query(
            `SELECT b.status FROM color_changes c JOIN batches b ON b.id = c.batch_id
              WHERE c.id = $1 FOR UPDATE OF c`,
            [attachment.owner_id]
          );
          if (locked.rows[0]?.status === "ARCHIVED") throw new AppError(409, "BATCH_ARCHIVED", "批次已归档，证据为只读");
